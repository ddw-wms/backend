...
export const getCustomers = async (req: Request, res: Response) => {
  try {
    ...
    // Query customers table for customer names (master data source)
    ...
    // If no customers found in customers table, fallback to outbound table
    if (result.rows.length === 0) {
      console.log('No customers in customers table, checking outbound table...');
      sql = `
      ...
      `;
      result = await query(sql, [warehouseId]);
      console.log('Outbound table rows:', result.rows.length);
    }
