...
export const getAllQCWSNs = async (req: Request, res: Response) => {
  try {
    const result = await query(
      "SELECT DISTINCT UPPER(TRIM(wsn)) as wsn FROM qc WHERE wsn IS NOT NULL AND wsn != '' ORDER BY wsn"
    );
...
export const getPendingInboundForQC = async (req: Request, res: Response) => {
  try {
    ...
    let sql = `
      SELECT i.id as inbound_id, i.wsn, i.inbound_date, i.vehicle_no,
             i.rack_no, i.product_serial_number, m.product_title, m.brand,
             m.cms_vertical, m.mrp, m.fsp, m.fkt_link, m.wid, m.fsn
      FROM inbound i
...
export const getQCList = async (req: Request, res: Response) => {
  try {
    ...
    if (search && search !== '') {
      whereConditions.push(`q.wsn = $${paramIndex}`);
      params.push(search);
      paramIndex++;
    }
