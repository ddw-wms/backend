...
export const getInventoryPipeline = async (req: Request, res: Response) => {
  try {
    ...
    /* 📦 AVAILABLE ONLY - Items that are inbound but not yet dispatched */
    if (availableOnly === 'true') {
      conditions.push(`NOT EXISTS (SELECT 1 FROM outbound WHERE outbound.wsn = i.wsn AND outbound.warehouse_id = i.warehouse_id)`);
    }
    ...
    if (stage && stage !== "all") {
      conditions.push(`(
        CASE
          ...
          WHEN q.id IS NOT NULL AND q.qc_status = 'Pass' THEN 'QC_PASSED'
          WHEN q.id IS NOT NULL AND q.qc_status = 'Fail' THEN 'QC_FAILED'
...
export const getInventoryDataForExport = async (req: Request, res: Response) => {
  try {
    ...
    // Stage filter – ab list ke stage strings use nahi; export ke mapped strings use:
    let stageFilter = '';
    if (stage && stage !== 'all') {
      if (stage === 'INBOUND_RECEIVED') {
        stageFilter = 'AND (o.id IS NULL AND p.id IS NULL AND q.id IS NULL)';
      } else if (stage === 'QC_PASSED' || stage === 'QC_PENDING' || stage === 'QC_FAILED') {
        stageFilter = 'AND (q.id IS NOT NULL AND p.id IS NULL AND o.id IS NULL)';
      } else if (stage === 'PICKING_PENDING' || stage === 'PICKING_COMPLETED') {
        stageFilter = 'AND (p.id IS NOT NULL AND o.id IS NULL)';
      } else if (stage === 'OUTBOUND_READY' || stage === 'OUTBOUND_DISPATCHED') {
